...
void GUI::connect_signals() {
	...
	QObject::connect(btnROnly, &QPushButton::clicked, this, [this]() {
		auto cWnd = new ContractReadOnlyGUI{ srv.get_contract() };
		cWnd->show();
		});
}
...
void GUI::reload_table(const vector<Subject>& all) {
	tableSubjects->clear();
	tableSubjects->setRowCount((int)all.size() + 1);
	tableSubjects->setColumnCount(4);
	tableSubjects->setItem(0, 0, new QTableWidgetItem(QString("Name")));
	tableSubjects->setItem(0, 1, new QTableWidgetItem(QString("Hours")));
	tableSubjects->setItem(0, 2, new QTableWidgetItem(QString("Type")));
	tableSubjects->setItem(0, 3, new QTableWidgetItem(QString("Teacher")));
	for (int i = 0; i < all.size(); i++) {
		QTableWidgetItem* name = new QTableWidgetItem(QString::fromStdString(all[i].get_name()));
		tableSubjects->setItem(i + 1, 0, name);
		QTableWidgetItem* hours = new QTableWidgetItem(QString::number(all[i].get_hours()));
		tableSubjects->setItem(i + 1, 1, hours);
		QTableWidgetItem* type = new QTableWidgetItem(QString::fromStdString(all[i].get_type()));
		tableSubjects->setItem(i + 1, 2, type);
		QTableWidgetItem* teacher = new QTableWidgetItem(QString::fromStdString(all[i].get_teacher()));
		tableSubjects->setItem(i + 1, 3, teacher);
	}
}
