...
void test_repolab(){
	...
	try {
		...
		assert(ex.get_message() == "0.000000");
	}
	repo.add_subject(Subject("sda", 4, "obl", "ana"));	
	const vector<Subject>& list = repo.get_all();
	assert(list[0].get_name() == "oop");
...
void alt_test_repolab() {
	RepoLab repo{ 1.0f };
	try {
		repo.add_subject(Subject{ "a",1,"b","c" });
		assert(false);
	}
	catch (RepoEx& ex) {
		assert(ex.get_message() <= "1.000000");
	}
	try {
		repo.delete_subject("aaa");
		assert(false);
	}
	catch (RepoEx& ex) {
		assert(ex.get_message() <= "1.000000");
	...
	try {
		repo.update_subject(Subject{ "a",1,"b","c" });
		assert(false);
	}
	catch (RepoEx& ex) {
		assert(ex.get_message() <= "1.000000");
